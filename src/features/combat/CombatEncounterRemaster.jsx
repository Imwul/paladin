...
export default function CombatEncounterRemaster({ character, setCharacter, onNavigate }) {
  ...
  const activeCombat = character.campaign?.combat?.status === 'active';
  const urgentCare = Boolean(healthState.pendingDeath || healthState.surgeryNeeded || ['pending', 'blocked', 'must_withdraw'].includes(healthState.majorWoundCourage?.status));
  const [workspace, setWorkspace] = useState(activeCombat ? 'combat' : urgentCare ? 'care' : 'combat');
  ...
  return <article className={`folio-page combat-ledger ${activeCombat ? 'combat-ledger--active' : ''} view-animate`}>
    ...
    <nav className="combat-workspace-tabs" aria-label="전투와 회복 작업 공간">
      <button type="button" className={workspace === 'combat' ? 'active' : ''} onClick={() => setWorkspace('combat')} aria-pressed={workspace === 'combat'}><Swords size={17} aria-hidden="true" /><span>개인 전투</span>{activeCombat && <StatusSeal tone="warning">진행 중</StatusSeal>}</button>
      <button type="button" className={workspace === 'care' ? 'active' : ''} onClick={() => setWorkspace('care')} aria-pressed={workspace === 'care'}><Bandage size={17} aria-hidden="true" /><span>상처와 회복</span>{untreatedWounds.length > 0 && <b>{untreatedWounds.length}</b>}</button>
      <button type="button" className={workspace === 'hazard' ? 'active' : ''} onClick={() => setWorkspace('hazard')} aria-pressed={workspace === 'hazard'}><AlertTriangle size={17} aria-hidden="true" /><span>기타 피해</span></button>
    </nav>

    <div className="combat-workspace" hidden={workspace !== 'combat'}>
      {!activeCombat && <SectionHeader index="I" title="개인 전투 준비" meta="Determination to Movement" action={<RulebookButton page={115} reason="Chapter 7 Combat" />} />}
      {!activeCombat && <Chapter18Encounter character={character} setCharacter={setCharacter} />}
      <Chapter7CombatEngine character={character} setCharacter={setCharacter} onNavigate={onNavigate} />
    </div>

    <div className="combat-workspace" hidden={workspace !== 'care'}>
      <SectionHeader index="II" title="상처와 응급처치" meta="Wounds · First Aid" action={<RulebookButton page={130} reason="Wounds and First Aid" />} />
      <div className="segmented-control combat-global-roll-mode" aria-label="치료 주사위 방식"><button type="button" className={rollMode === 'automatic' ? 'active' : ''} onClick={() => setRollMode('automatic')}>앱 굴림</button><button type="button" className={rollMode === 'manual' ? 'active' : ''} onClick={() => setRollMode('manual')}>직접 굴림</button></div>
      ...
      <SectionHeader index="III" title="주간 회복" meta="Chirurgery · Sunday Noon" action={<RulebookButton page={132} reason="Healing and Chirurgery" />} />
    ...
    <div className="combat-workspace" hidden={workspace !== 'hazard'}>
      <SectionHeader index="IV" title="기타 피해" meta="Falls · Fire · Poison · Suffocation" action={<RulebookButton page={134} reason="Other Damage" />} />
