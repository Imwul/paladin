    ...
    <article className="folio-page reputation-ledger view-animate">
      ...
      <div className="standing-register">
        {Object.entries(character.standings || {}).map(([key, value]) => (
          <LedgerRow key={key} label={STANDING_LABELS[key] || key} meta={key.toUpperCase()} value={value} accent={Number(value) >= 16} />
        ))}
      </div>
  ...
  return (
    <article className="folio-page reputation-ledger view-animate">
      ...
      <section className="glory-totals" aria-label="영광 합계">
        <div><span>TOTAL</span><strong>{(character.gear?.gloryTotal || 0).toLocaleString()}</strong><small>누적 영광</small></div>
        <div><span>ANNUAL</span><strong>{(character.gear?.gloryThisGame || 0).toLocaleString()}</strong><small>이번 해</small></div>
        <div><span>INHERITED</span><strong>{inherited.toLocaleString()}</strong><small>계승 기록</small></div>
      </section>
