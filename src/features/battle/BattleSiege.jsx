...
const SkirmishFlow = ({ character, setCharacter, run }) => {
  const skirmish = character.campaign?.skirmish;
  const [round, setRound] = useState({ outcome: 'draw', enemiesDefeated: 0, note: '' });
  ...
  const phaseContent = () => {
    ...
    if (skirmish.phase === 'melee') {
      ...
      return <><RuleNote page="pp.138-139">개인 공격·피해·Glory는 Chapter 7 전투 장부에서 정상 처리합니다. 이 장부에는 그 결과만 기록하며, 지휘 수정 {modifier >= 0 ? '+' : ''}{modifier}는 {number === 1 ? '이번 첫 라운드의 전투 기술에만 적용됩니다' : '더 이상 적용되지 않습니다'}.</RuleNote><div className="battle-form-grid"><SelectField label={`${number}라운드 개인 전투 결과`} value={round.outcome} onChange={value => setRound(previous => ({ ...previous, outcome: value }))} options={[{value:'victory',label:'개인 전투 승리'},{value:'draw',label:'교전 지속 또는 결판 없음'},{value:'defeat',label:'개인 전투 패배'},{value:'withdrawal',label:'교전에서 이탈'}]} /><NumberField label="이번 라운드에 쓰러뜨린 적" value={round.enemiesDefeated} min={0} onChange={value => setRound(previous => ({ ...previous, enemiesDefeated: value }))} /></div><button type="button" className="primary-command" onClick={() => updateCharacter(previous => recordSkirmishMeleeRound(previous, round))}><ChevronRight size={17} aria-hidden="true" />라운드 결과 기록</button></>;
    }
...
const BattleFlow = ({ character, setCharacter, run }) => {
  ...
  const phaseContent = () => {
    ...
    if (battle.phase === 'first_charge') return <><RuleNote page="p.143">말과 마상창을 갖춘 기사만 첫 돌격에 참가합니다. 돌격은 전투 1라운드로 셉니다.</RuleNote><label className="battle-binary"><input type="checkbox" checked={charge} onChange={event => setCharge(event.target.checked)} /><span>첫 돌격 참가</span></label><button type="button" className="primary-command" onClick={() => updateCharacter(previous => resolveFirstCharge(previous, { participates: charge }))}><Dices size={17} aria-hidden="true" />첫 돌격 해결</button></>;
    ...
    if (battle.phase === 'melee_action') {
      ...
      return <>
        ...
        {specialPrepared && <><ResultBlock title={`${pending.specialEvent.type} · ${pending.specialEvent.enemy.name}`} tone="active"><p>Battle Enemy {pending.specialEvent.enemyRoll} · 무기 {pending.specialEvent.enemy.primarySkill} · 갑옷 {pending.specialEvent.enemy.armor}</p></ResultBlock><RuleNote page="pp.145-146">이 장부를 떠나도 현재 단계가 저장됩니다. Chapter 7 개인 전투를 원하는 만큼 진행한 뒤 결과와 그 전투에서 얻은 Glory만 기록하세요.</RuleNote><div className="battle-form-grid"><SelectField label="특별 개인 전투 결말" value={actionDetails.specialCombatResult} onChange={value => setActionDetails(previous => ({ ...previous, specialCombatResult: value }))} options={[{value:'defeated',label:'상대를 쓰러뜨림'},{value:'captured',label:'상대를 포획함'},{value:'gave_up',label:'포기하고 대전투로 복귀'}]} /><NumberField label="개인 전투 추가 Glory" value={actionDetails.specialCombatGlory} min={0} onChange={value => setActionDetails(previous => ({ ...previous, specialCombatGlory: value }))} /></div></>}
...
const SiegeFlow = ({ character, setCharacter, run }) => {
...
export default function BattleSiege({ character, setCharacter }) {
  ...
  return (
    <article className="folio-page battle-ledger view-animate">
      ...
      {mode === 'skirmish' ? <>{(!skirmish || skirmish.status === 'complete') && <SkirmishSetup character={character} setCharacter={setCharacter} run={run} />}{skirmish && <SkirmishFlow character={character} setCharacter={setCharacter} run={run} />}</> : mode === 'battle' ? <>{(!battle || battle.status === 'complete') && <BattleSetup character={character} setCharacter={setCharacter} run={run} />}{battle && <BattleFlow character={character} setCharacter={setCharacter} run={run} />}</> : <>{(!siege || siege.status === 'complete') && <SiegeSetup character={character} setCharacter={setCharacter} run={run} />}{siege && <SiegeFlow character={character} setCharacter={setCharacter} run={run} />}</>}
    </article>
  );
}
