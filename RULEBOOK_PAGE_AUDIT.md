...
완전한 규칙서 등가물로 판정하지 않습니다. Chapter 8의 소규모 교전·대규모 전투·공성은 종단 실행할 수 있지만, 고급 개인 전투 일부, 부와 거래, 외국 문화 생성, 적·생물 실행, Chapter 19 완전 모험은 출시 차단 범위입니다. 겨울은 원문 순서 전체를 실행하지만 GM 판단이 필요한 결과는 명시적 미결 상태로 남깁니다.
...
| 149 | 148 | Chapter Eight: Mass Combat | **구현** · Table 8-9·8-10 승패, 군 전체 운명, 전리품·몸값·Glory 확정 | 몸값 금액 산정은 Chapter 12 미결 | pending economy로 손실 없이 보존 |
